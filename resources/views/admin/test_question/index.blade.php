                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                ...
                                <a href="{{ route('admin.question.add') }}" class="btn btn-primary btn-flat float-right">Add
                                    Question</a>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
...
@section('script')
    ...
    <script>
        $(function() {
            var initaltable = $('#question_table');
            initaltable.DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.get-question-list') }}",
                    type: "POST",
                    data: function(d) {
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(data) {
                            $('#question_table').DataTable().draw();
                            $(document).Toasts('create', {
                                class: 'bg-success',
                                title: 'Delete',
                                subtitle: 'State Delete',
                                body: 'State Delete Succefully!.'
                            });
                            $('#toastsContainerTopRight').delay(5000).fadeOut('slow');
                        },
                        error: function(xhr, ajaxOptions, thrownError) {                            
                            $('#question_table').DataTable().reload();
                            $(document).Toasts('create', {
                                class: 'bg-danger',
                                title: 'Opps!',
